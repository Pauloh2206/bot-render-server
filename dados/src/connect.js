...
http.createServer((req, res) => {
    ...
    console.log(`✅ Servidor de Health Check rodando na porta ${process.env.PORT || 3000} em 0.0.0.0`);
});
...
async function startNazu() {
    // Garantia de Pastas com Caminhos Absolutos para o Render
    const tempPath = path.join(__dirname, '..', '..', 'temp');
    const sessionPath = path.join(__dirname, '..', '..', 'dados', 'database', 'auth_info_baileys');
    const databasePath = path.join(__dirname, '..', '..', 'dados', 'database');

    [tempPath, sessionPath, databasePath].forEach(dir => {
...
process.on('uncaughtException', async (error) => {
    ...
    if (error.message.includes('ENOSPC') || error.message.includes('ENOMEM')) {
        console.warn('⚠️ Limpeza de emergência disparada por falta de espaço ou memória!');
        try {
            const tempPath = path.join(__dirname, '..', '..', 'temp');
            if (fs.existsSync(tempPath)) {
                const files = fs.readdirSync(tempPath);
                files.forEach(file => {
                    try { fs.unlinkSync(path.join(tempPath, file)); } catch (e) {}
                });
